...
class imageDataset(Dataset):
    def __init__(self, mapping, transform=None,target_transform=None, Test=False):
        ...
        self.Test = Test
        print(self.Test)
        pair_list = os.listdir(mapping)
        self.nSample = len(pair_list)

        for pair in pair_list:
            img = 'data/downstream_imgs/'+pair
            # label = items[1] # No blank in the middle of the label string.
            label_file = 'data/downstream_gt/'+pair.split('.')[0]+".txt"
            label = open(label_file).readlines()[0]
    ...
    def __getitem__(self, index):
        ...
        #_, img = cv2.threshold(img, 120, 255, cv2.THRESH_OTSU + cv2.THRESH_BINARY)
